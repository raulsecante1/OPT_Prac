...
class MinimalKGA:
    def __init__(self, G, K, pop_size=100, mutation_rate=0.01):
        self.G = G
        self.K = K
    ...
    def fitness(self, chrom):
        ...
        for u, v, w in self.G.edges(data='capacity', default=1):
            ...
            if chrom[iu] != chrom[iv]:
                cost += w
        counts = np.bincount(chrom, minlength=self.K)
        missing = np.sum(counts == 0)
        cost += missing * penalty
...
def main():
    ...
    if read_file:
        ...
        graph = minmal_k_cut.make_random_graph()

    GA_start = time.perf_counter()

    mcga = MinimalKGA(graph, 2)
    mcga.run()
