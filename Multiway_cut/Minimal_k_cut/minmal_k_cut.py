...
def make_random_graph(n = 150, p = 0.1, w_min=1, w_max=30):
    '''
...
def main():
    ...
    mkc_start = time.perf_counter()
    result = min_k_cut_2minus2dk(graph, 2)
    mkc_stop = time.perf_counter()

    print("Elapsed time for mkc: ", mkc_stop - mkc_start)

    cost = 0
    for elem in result:
        cost += cut_weight(graph, elem)
    print ("result = ", result, " cost = ", cost, "\n")
