...
//std::string path = "./public/simple.gr";
std::string path = "C:/Users/aragr/Documents/ucm/OPT25-26/AG/public/vc-exact_019.gr";
...
void get_res(const std::vector<std::vector<int>>& vecs) {
    int minimum_v = -1;
    int maximum_v = 0;
    int max_count = 0;
    int min_count = 0;
    ...
    for (std::vector<int> vec : vecs) {
        ...
        if (minimum_v > count || minimum_v < 0) {
            minimum_v = count;
            min_count = 1;
        }
        else if (minimum_v == count) {
            min_count++;
        }
        if (maximum_v < count) {
            maximum_v = count;
        ...
        else if (maximum_v == count) {
            max_count++;
        }
    }
    std::cout << "best solution: " << minimum_v << " of " << (double)min_count/vecs.size()*100.0 << "% , worst solution:  " << maximum_v << " of " << (double)max_count/vecs.size()*100.0 << "% \n";
}
...
std::vector<std::vector<int>> dual_point_selection(const std::vector<std::vector<int>>& parents, bool debug = false) {
    /*
...
//#pragma omp parallel for
    for (int ind = 0; ind < num_population; ind+=2) {
        ...
        if (debug) {
            std::cout << "parent 1 is: ";
            print_particle(child1);
            std::cout << "parent 2 is: ";
            print_particle(child2);
        }
        for (int i = p1; i <= p2; ++i) {
...
std::vector<std::vector<int>> torneo_selection(const std::vector<std::vector<int>>& current_generation, const BipartiteGraph graph, bool debug = false) {
    /*
    ...
    for (int ind = 0; ind < num_population*2; ind += 2) {
...
int main(){
    ...
    for (int ind_iter = 0; ind_iter < num_iteration; ind_iter++) {                 // condition phase
        ...
        if (debug) std::cout << ind_iter << "iteration run\n";

        child_particles = dual_point_selection(parent_particles);                               // 2 points crossover
        if (debug) std::cout << ind_iter + 1 << "-iterate crossover over\n";                          //

        mutuation(0.01f, child_particles);                                         // 1% mutuation
        if (debug) std::cout << ind_iter+1 << "-iterate mutation over\n";         //
        ...
        if (debug) std::cout << ind_iter+1 << "-iterate 2 zones selection over";                                  //
